...
$sql = "SELECT * FROM course WHERE cour_category = 'Science and Discovery'";
...
if (isset($_POST['search'])) {
    $search_query ="SELECT *FROM course WHERE cour_name LIKE '%".$_POST['search']."%' AND WHERE cour_category = 'Science and Discovery'";
    $result = executeQuery($search_query);
    $starting_num = 0;
} else {
    //Retreive selected result from the table and display them on page
    $sql = "SELECT *,(SELECT AVG(rate_course_value) FROM rating r WHERE (r.cour_id = c.cour_id)) AS avg_rating FROM course c, teacher t WHERE (c.teac_id = t.teac_id) AND (c.cour_category = 'Science and Discovery') ORDER BY RAND()";
...
<head>
    ...
    <title>Science and Discovery Courses</title>
</head>
<body>
