		...
		<div class="card">
			<div class="card-body">
				<table class="table table-bordered" id='table'>
					...
					<tbody> 
					<?php
					$qry = $conn->query("SELECT course.course_category, course.course_description, course.course_create_date, course.course_id, course.course_title, count(*) FROM course LEFT JOIN course_chapter ON (course.course_id = course_chapter.chapter_id) GROUP BY course.course_id");
					$i = 1;
					if($qry->num_rows > 0){
					...
					<tr>
						...
						<td>
							...
							<button class="btn btn-sm btn-outline-primary editbtn" data-id="<?php echo $row['course_id']?>" type="button"><i class="fa fa-edit"></i> Edit</button>
		...
		$(document).on("click",".editbtn",function(){
			$('#edit_quiz').modal('show');
				$tr = $(this).closest('tr');
					...
					$('#course_title').val (data[1]);
					$('#course_category').val (data[2]);
					$('#course_description').val(data[3]);
		});
